...
export function weapon_passive() {
            ...
            if (typeof passiveCooldown == `number`) {
                switch (true) {
                    ...
                    case currentLore && currentLore.some((line) => line === "§r§d[Passive] SPEED buff When Holding"):
                        ...
                        break;
                    case currentLore &&
                        currentLore.some((line) => line === "§r§d[Passive] SLOW Nearby Enemies Over Time"):
    ...
    Minecraft.world.afterEvents.entityHitEntity.subscribe((event) => {
        ...
        if (player.typeId != "minecraft:player")
            return;
        switch (true) {
            case currentLore && currentLore.some((line) => line === "§r§d[Passive] Increased Damage While Poisoned"):
                ...
                break;
            case currentLore &&
                currentLore.some((line) => line === "§r§d[Passive] Extra Damage To Illager Type Mobs"):
                const ILLAGER_MOBS = [
                    "minecraft:pillager",
    ...
    Minecraft.world.afterEvents.entityDie.subscribe((event) => {
        ...
        let currentLore = playerHeld.getLore();
        switch (true) {
            case currentLore && currentLore.some((line) => line === "§r§d[Passive] Drop SOULS from defeated enemies"):
