...
export function weapon_onhit() {
    ...
    Minecraft.world.afterEvents.entityHitEntity.subscribe((event) => {
        ...
        if (onHitCooldown === 0) {
            switch (true) {
                case currentLore && currentLore.some((line) => line === "§r§d[On-Hit] Steal A PORTION of Enemy Health"):
                    ...
                    player.setDynamicProperty(`on_hit_cooldown`, onHitCooldown + 1);
                    break;
                case currentLore && currentLore.some((line) => line === "§r§d[On-Hit] Infect Hurt With Weakness"):
                    hurtEntity.addEffect(`weakness`, 60, { amplifier: 2 });
                    player.setDynamicProperty(`on_hit_cooldown`, onHitCooldown + 1);
                    break;
                case currentLore && currentLore.some((line) => line === "§r§d[On-Hit] Poison The Enemy"):
