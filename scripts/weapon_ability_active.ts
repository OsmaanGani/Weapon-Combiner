...
export function weapon_active() {
  Minecraft.system.runInterval(() => {
    Minecraft.world.getAllPlayers().forEach((player: Minecraft.Player) => {
      ...
      player.dimension.getEntities().forEach((entity: Minecraft.Entity) => {
  ...
  Minecraft.world.afterEvents.itemUse.subscribe((event) => {
    let player = event.source as Minecraft.Player;
      ...
      switch (true) {
                    ...
                    if (Math.random() < 0.06) {
                      replaceableBlocks.forEach((blockType) => {
                        entity.entity.runCommand(
                          `fill ~${x} ~-4 ~${z} ~${x} ~3 ~${z} minecraft:sculk replace minecraft:${blockType}`
                        );
                        if (Math.random() < 0.005) {
                          entity.entity.runCommand(
                            `fill ~${x} ~-2 ~${z} ~${x} ~ ~${z} minecraft:sculk_sensor replace air`
                          );
                        }
                        if (Math.random() < 0.005) {
                          entity.entity.runCommand(
                            `fill ~${x} ~-2 ~${z} ~${x} ~ ~${z} minecraft:sculk_catalyst replace air`
                          );
                        }
                        if (Math.random() < 0.002) {
                          entity.entity.runCommand(
                            `fill ~${x} ~-2 ~${z} ~${x} ~ ~${z} minecraft:sculk_shrieker replace air`
                          );
                        }
                      });
                    }
        ...
        case currentLore.some((line) => line === "§r§d[Active] Interact To BOUNCE foes"):
          player.dimension
            ...
            .forEach((entity: Minecraft.Entity) => {
              if (
