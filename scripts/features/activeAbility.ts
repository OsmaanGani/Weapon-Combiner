...
export class ActiveAbility {
  ...
  private static _unleashSouls: AbilityFn = (p) => {
  ...
  private static _freezeEntities: AbilityFn = (p) => {
  ...
  private static _poisonToRegen: AbilityFn = (p) => {
  ...
  private static _simpleHealthBoost(amplifier: number): AbilityFn {
    return (p) => {
  ...
  private static _dashForward: AbilityFn = (p) => {
    let { x, y, z } = p.getViewDirection();
    p.applyKnockback(x, z, 3, 0.1);
  };

  private static _bounceFoes: AbilityFn = (p) => {
  ...
  private static _divineLightning: AbilityFn = (p) => {
  ...
  private static _summonFireballs: AbilityFn = (p) => {
  ...
  private static _doubleJump: AbilityFn = (p) => {
  ...
  private static _dashFire: AbilityFn = (p) => {
  ...
  private static _curseEntities: AbilityFn = (p) => {
    p.dimension.getEntities({ maxDistance: 5, location: p.location }).forEach((entity: Entity) => {
      p.dimension.spawnEntity("bey:radius_entity", p.location);
